...
class BookClass {
    ...
    static async add({ name, price, githubRepo }) {
        ...
        if(!slug) {
            throw new Error(`Error with slug generation with name: ${name}`)
        }
    ...
    static async syncContent({ id, user, request }) {
        ...
        if(lastCommitSha === book.githubLastCommitSha) {
            throw Error('No change in content')
        }
        ...
        await Promise.all(
            mainFolder.data.map(async (f) => {
                ...
                const data = frontmatter(Buffer.from(chapter.data.content,'base64').toString('utf-8'))
    ...
    static async buy({ book, user, stripeCharge }) {
        ...
        try {
            await addToMainchimp({ email: user.email, listName: 'purchased' })
        } catch (error) {
            logger.error('Buy book error:', error);
            console.error('models-Book-Buy book error:', error);
        }
...
async function getPurchasedBooks({ purchasedBookIds }) {
