...
const he = require('he')
const hgjs = require('highlight.js')
...
function markdownToHtml(content) {
    ...
    renderer.heading = (text, level) => {
        ...
        switch (level) {
            case 2:
                ret_val = `<h${level} class="chapter-section" style="color: #222; font-weight: 400;">
                    <a
                        name="${escapedText}"
                        href="#${escapedText}"
                        style="color: #222;"
            ...
            case 4:
                ret_val = `<h${level} style="color: #222;">
                        <a
                            name="${escapedText}"
                            href="#${escapedText}"
                            style="color: #222;"
            ...
            default:
                ret_val = `<h${level} style="color: #222; font-weight: 400;">${text}</h${level}>`
                break;
        }
    ...
    marked.setOptions({
        ...
        highlight(code, lang) {
            if(!lang) {
                return hgjs.highlightAuto(code).value
            }

            return hgjs.highlight(lang, code).value
        },
    })
...
class ChapterClass {
    static async getBySlug({ bookSlug, chapterSlug, userId, isAdmin }) {
        ...
        console.log(`Chapter-ChapterClass-getBySlug userId: ${userId}`)
        console.log(`Chapter-ChapterClass-getBySlug bookSlug: ${bookSlug}`)
        console.log(`Chapter-ChapterClass-getBySlug chapter: ${chapter}`)
