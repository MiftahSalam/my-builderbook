...
function createSession({ userId, bookId, bookSlug, userMail, redirectUrl }) {
    console.log("server-stripe-createSession", userId, bookId, bookSlug, userEmail, redirectUrl);
    ...
    return stripeInstance.checkout.sessions.create({
        ...
        success_url: `${ROOT_URL}/stripe/checkout-complete/{CHECKOUT_SESSION_ID}`,
...
function stripeCheckoutCallback({ server }) {
    server.get('/stripe/checkout-complete/:sessionId', async (req, res) => {
        ...
        try {
            ...
            console.log("server-stripe-stripeCheckoutCallback user", user);
            ...
            logger.error(err);
            console.log("server-stripe-stripeCheckoutCallback error", error);
            res.redirect(
                `${ROOT_URL}${session.metadata.redirectUrl}?error=${error.message || error.toString()}`
            )
        }
    })
}
